...
std::string read_file(const std::string& filename)
{
    ...
    log_event("Error: File not found - " + filename);
    return "ERROR";
}
...
{
    //  Save file with proper structure (student name, date, key, and data)
    //  file format
    //  Line 1: student name
    ...
    if (file)
    {
        ...
        file << (1900 + local_time.tm_year) << "-"
            ...
            << std::setw(2) << std::setfill('0') << local_time.tm_mday << std::endl;
...
void log_event(const std::string& message) {
    ...
    if (log) {
        ...
        std::tm local_time;
        localtime_s(&local_time, &now); // safer version

        log << std::put_time(&local_time, "%Y-%m-%d %H:%M:%S") << " - " << message << std::endl;
    }
}
...
int main()
{
    ...
    log_event("Encrypted data saved to: " + encrypted_file_name);
    ...
    log_event("Decrypted data saved to: " + decrypted_file_name);
